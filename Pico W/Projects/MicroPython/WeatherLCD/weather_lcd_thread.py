...
from EasyThread import (
    EasyThread,
)  # from https://github.com/jblanked/RaspberryPi/blob/main/Pico%20W/Libraries/Python/EasyThread.py
...
gc.collect()
weather = WeatherLCD("your_wifi", "your_password")
try:
    if not weather.start():
        raise Exception("Failed to start weather system")
    ...
    print(f"Error starting weather: {e}")
    ...
    try:
        while not stop_thread:
            ...
            try:
                ...
                sleep(1)  # Add delay to prevent tight loop on error
    except Exception as e:
        print(f"Fatal error in handle_weather: {e}")
    finally:
        print("Weather handler stopped")


time_thread = None
failed_attempts = 0
try:
    time_thread = EasyThread(handle_weather, ())
    time_thread.start()
    ...
    while True:
        ...
        try:
            # check thread status
            if not stop_thread and not time_thread.is_alive():
                print("Time thread stopped, restarting...")
                weather.lcd.clear()
                time_thread = EasyThread(handle_weather, ())
                time_thread.start()

            if not weather.run():
                print("Weather run failed, retrying in 5 seconds...")
                failed_attempts += 1
                sleep(5)
            ...
            weather.lcd.write("Error occurred", True, 0, 0)
            weather.lcd.write(str(e)[:16], False, 0, 1)  # Truncate to 16 chars for LCD
